...
local t = Def.ActorFrame {
	...
	OffCommand = function(self)
		self:bouncebegin(0.2):xy(0 , -560):diffusealpha(0)
...
local function littlebits(i)
	local t = Def.ActorFrame {
		LoadFont("Common Large") .. {
			InitCommand = function(self)
				self:xy(frameX + offsetX, frameY + 98 + txtDist * i):halign(0):valign(0):zoom(0.35):maxwidth(155 / 0.55)
			end,
			SetCommand = function(self)
				--skillset name
				if song and steps then
		...
		LoadFont("Common Large") .. {
			InitCommand = function(self)
				self:xy(frameX + 240, frameY + 98 + txtDist * i):halign(1):valign(0):zoom(0.35):maxwidth(110 / 0.55)
			end,
			SetCommand = function(self)
				if song and steps then
...
t[#t + 1] = LoadFont("Common Normal") .. {
	Name = "StepsAndMeter",
	InitCommand = function(self)
		self:xy(frameX + offsetX, frameY + offsetY + 44):zoom(0.35):halign(0):maxwidth(350)
	end,
	SetCommand = function(self)
		steps = GAMESTATE:GetCurrentSteps()
		if steps ~= nil then
